...
handle_cwd(struct vsf_session* p_sess)
{
  ...
  if (!vsf_access_check_file(&p_sess->ftp_arg_str, kVSFDirChange))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
...
handle_retr(struct vsf_session* p_sess)
{
  ...
  if (!vsf_access_check_file(&p_sess->ftp_arg_str, kVSFFileGet))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
  ...
  p_sess->crc = trans_ret.crc;
  p_sess->crc2 = trans_ret.crc2;
...
handle_dir_common(struct vsf_session* p_sess, int full_details, int stat_cmd)
{
  ...
  if (!str_isempty(&s_filter_str))
  {
    ...
    if (!vsf_access_check_file(&s_filter_str, kVSFFileView))
    {
      vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
      return;
    }
...
handle_upload_common(struct vsf_session* p_sess, int is_append, int is_unique)
{
  ...
  str_copy(&p_sess->log_str, &p_sess->ftp_arg_str);
  prepend_path_to_filename(&p_sess->log_str);
  if (!vsf_access_check_file(p_filename, kVSFFilePut))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
...
handle_mkd(struct vsf_session* p_sess)
{
  ...
  str_copy(&p_sess->log_str, &p_sess->ftp_arg_str);
  prepend_path_to_filename(&p_sess->log_str);
  if (!vsf_access_check_file(&p_sess->ftp_arg_str, kVSFDirCreate))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
...
handle_rmd(struct vsf_session* p_sess)
{
  ...
  str_copy(&p_sess->log_str, &p_sess->ftp_arg_str);
  prepend_path_to_filename(&p_sess->log_str);
  if (!vsf_access_check_file(&p_sess->ftp_arg_str, kVSFDirDel))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
...
handle_dele(struct vsf_session* p_sess)
{
  ...
  str_copy(&p_sess->log_str, &p_sess->ftp_arg_str);
  prepend_path_to_filename(&p_sess->log_str);
  if (!vsf_access_check_file(&p_sess->ftp_arg_str, kVSFFileDel))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
...
handle_rnfr(struct vsf_session* p_sess)
{
  ...
  resolve_tilde(&p_sess->ftp_arg_str, p_sess);
  if (!vsf_access_check_file(&p_sess->ftp_arg_str, kVSFFileRename))
  {
...
handle_size(struct vsf_session* p_sess)
{
  ...
  int retval;
  resolve_tilde(&p_sess->ftp_arg_str, p_sess);
  if (!vsf_access_check_file(&p_sess->ftp_arg_str, kVSFFileView))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
...
handle_site_chmod(struct vsf_session* p_sess, struct mystr* p_arg_str)
{
  ...
  if (!vsf_access_check_file(&s_chmod_file_str, kVSFFileChmod))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
...
{
  (void) p_arg_str;
  struct mystr who_str = INIT_MYSTR;
  vsf_db_get_session_list(&who_str);
  vsf_banner_write(p_sess, &who_str, FTP_SITEHELP);
  str_free(&who_str);
...
handle_mdtm(struct vsf_session* p_sess)
{
  ...
  resolve_tilde(&p_sess->ftp_arg_str, p_sess);
  /* Note: A user which is allowed to append should be allowed to change the
   *       modification time too.
   */
  if (!vsf_access_check_file(&p_sess->ftp_arg_str, kVSFFileResume))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
  }
...
handle_stat(struct vsf_session* p_sess)
{
  ...
  vsf_cmdio_write_raw(p_sess,
    "     vsftpdx " VSF_VERSION " - secure, fast, stable\r\n");
