...
vsf_sysutil_get_ipv6_sock(void)
{
  int retval = 0;
#ifdef VSF_BUILD_IPV6
  retval = socket(PF_INET6, SOCK_STREAM, IPPROTO_TCP);
  if (retval < 0)
  {
    die("socket");
  }
#else
  bug("not compiled with ipv6 support");
#endif
  return retval;
}
...
unsigned short 
vsf_sysutil_sockaddr_get_port(struct vsf_sysutil_sockaddr* p_sockptr)
{
  if (p_sockptr->u.u_sockaddr.sa_family == AF_INET)
  {
    return p_sockptr->u.u_sockaddr_in.sin_port;
  }
#ifdef VSF_BUILD_IPV6
  else if (p_sockptr->u.u_sockaddr.sa_family == AF_INET6)
  {
    return p_sockptr->u.u_sockaddr_in6.sin6_port;
  }
#endif
...
int
vsf_sysutil_is_group_member(gid_t gid)
{
  int ngroups = (int) getgroups(0, NULL);
...
vsf_sysutil_running_as_root(void)
{
    ...
    return (getuid() == VSFTP_ROOT_UID ||
      vsf_sysutil_is_group_member(VSFTP_ROOT_GID) ||
      vsf_sysutil_is_group_member(VSFTP_ADMIN_GID));
  #else
    return (getuid() == VSFTP_ROOT_UID);
  #endif
}
